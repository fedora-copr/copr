...
def chroot_edit(username, coprname, chrootname):
    ...
    if flask.g.user.can_build_in(copr):
        return flask.render_template('coprs/detail/edit_chroot.html', form=form, copr=copr, chroot=chroot)
    else:
        return page_not_found("You are not allowed to modify chroots in project ${0}." % coprname)
...
def chroot_update(username, coprname, chrootname):
    ...
    if form.validate_on_submit() and flask.g.user.can_build_in(copr):
        ...
        return flask.redirect(flask.url_for('coprs_ns.copr_edit', username = username, coprname = copr.name))
    else:
