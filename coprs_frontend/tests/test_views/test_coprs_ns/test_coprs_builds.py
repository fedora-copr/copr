...
class TestCoprShowBuilds(CoprsTestCase):
    def test_copr_show_builds(self, f_users, f_coprs, f_mock_chroots, f_builds, f_db):
...
class TestCoprAddBuild(CoprsTestCase):
    ...
    def test_copr_owner_can_add_build(self, f_users, f_coprs, f_mock_chroots, f_db):
    ...
    @TransactionDecorator('u1')
    def test_copr_allowed_user_can_add_build(self, f_users, f_coprs, f_mock_chroots, f_copr_permissions, f_db):
...
class TestCoprCancelBuild(CoprsTestCase):
    ...
    @TransactionDecorator('u1')
    def test_copr_build_submitter_can_cancel_build(self, f_users, f_coprs, f_mock_chroots, f_builds, f_db):
    ...
    def test_copr_build_non_submitter_cannot_cancel_build(self, f_users, f_coprs, f_mock_chroots, f_builds, f_db):
        self.db.session.add_all([self.u1, self.c1, self.b1])
        r = self.test_client.post('/coprs/{0}/{1}/cancel_build/{2}/'.format(self.u1.name, self.c1.name, self.b1.id),
                  data = {},
                  follow_redirects = True)
