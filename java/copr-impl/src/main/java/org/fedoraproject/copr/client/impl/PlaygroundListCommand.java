 ...
 */
public class PlaygroundListCommand
    extends RpcCommand<PlaygroundListResult>
{
    public PlaygroundListCommand( PlaygroundListRequest request )
    {
    }
    ...
    @Override
    protected PlaygroundListResult parseResponse( JsonObject response )
    {
        List<ProjectId> projects = new ArrayList<>();

        JsonArray repos = response.get( "repos" ).getAsJsonArray();
        for ( JsonElement element : repos )
        {
            JsonObject repo = element.getAsJsonObject();

            ProjectId id = new ProjectId();
            id.setUserName( repo.get( "username" ).getAsString() );
            id.setProjectName( repo.get( "coprname" ).getAsString() );

            projects.add( id );
        }

        return new DefaultPlaygroundListResult( projects );
    }
}
