...
class PulpStorage(Storage):
    ...
    def fork_project(self, src_fullname, dst_fullname, builds_map):
        ...
        for chroot, src_dst_dir in builds_map.items():
            ...
            # TODO It should be a dirname here
            ...
            if not self.publish_repository(chroot):
    ...
    def _fork_build(self, src_build_id, dst_build_id, src_owner, src_project,
                    dst_owner, dst_project, dirname, chroot):
        # pylint: disable=too-many-positional-arguments
        ...
        with TemporaryDirectory(prefix="copr-fork-") as tmp:
            ...
            for rpm in rpms:
                rpm = rpm["location_href"]

                # TODO dynamically figure out the baseurl
                # TODO Move to pulp.py
                baseurl = "http://pulp:80/pulp/content"
        ...
        return self.create_repository_version(chroot, result).ok
