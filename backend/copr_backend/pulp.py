...
class BatchedAddRemoveContent:
    ...
    """
    def __init__(self, repo_href, rpms_to_add, rpms_to_remove, backend_opts=None, noop=False):
        self.repo_href = repo_href
        self.rpms_to_add = rpms_to_add
    ...
    def options(self):
        ...
        for key in self.redis.keys(self.key_pattern):
            ...
            if len(self.notify_keys) >= MAX_IN_BATCH - 1:  # one is ours!
                self.log.info("Batch add-remove limit %s reached, skip the rest",
                              MAX_IN_BATCH)
                break
...
class PulpClient:
    ...
    def try_lock(self, batch):
        ...
        while True:
            ...
            try:
                ...
                with lock(self.opts.directory, lockdir=lockdir, timeout=5, log=self.log):
                    response = self.modify_repository_content_locked(batch)
                    data = response.json()
                    if response.ok and data["state"] == "completed":
                        ...
                        self.log.info("Task %s failed due to: %s", data["pulp_href"], data["error"])
    ...
    def modify_repository_content(self, repository, add_content_units=[], remove_content_units=[]):
        ...
        data = {"add_content_units": add_content_units, "remove_content_units": remove_content_units}
    ...
    def add_content(self, repository, artifacts):
        ...
        https://pulpproject.org/pulp_rpm/restapi/#tag/Repositories:-Rpm/operation/repositories_rpm_rpm_modify
        """
        batch = BatchedAddRemoveContent(repository, artifacts, None, self.opts)
        ...
        try:
            ...
            self.log.info("Pulp: Failed to add content: %s %s", repository, artifacts)
    ...
    def delete_content(self, repository, artifacts):
        ...
        https://pulpproject.org/pulp_rpm/restapi/#tag/Repositories:-Rpm/operation/repositories_rpm_rpm_modify
        """
        batch = BatchedAddRemoveContent(repository, None, artifacts, self.opts)
        # Put our task to Redis DB and allow _others_ to process our
        # own task.  This needs to be run _before_ the lock() call.
        batch.make_request()
        try:
            ...
            self.log.info("Pulp:Failed to delete content %s (%s)", repository, artifacts)
