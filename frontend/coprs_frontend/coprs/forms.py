...
class TarballValidator:
    """
    Validate that the uploaded file looks like a .tar.gz tarball.
    """
    def __init__(self, message=None):
        if not message:
            message = "You can only upload .tar.gz files"
        self.message = message
    ...
    def __call__(self, form, field):
        ...
        if not file_storage.filename.lower().endswith(".tar.gz"):
            raise wtforms.ValidationError(self.message)
...
class BuildFormRpmUploadFactory:
    ...
    def __new__(cls, active_chroots):
        form = _get_build_form(active_chroots, BaseForm)
        form.tarball = FileField('tarball', validators=[
            FileRequired(),
            TarballValidator()])
        form.name = wtforms.StringField('name', validators=[
            wtforms.validators.DataRequired()])
        form.version = wtforms.StringField('version', validators=[
            wtforms.validators.DataRequired()])
