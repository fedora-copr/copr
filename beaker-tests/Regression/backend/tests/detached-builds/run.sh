...
rlJournalStart
	...
	echo "sleep 8"
        sleep 8 # downloading srpm to builder is taking place, mockchain is not running yet

        PID1=`docker exec copr-backend /bin/sh -c 'pgrep mockchain'`
	echo "pid1:" $PID1
	...
	echo "Restart backend"
        ...
        PID2=`docker exec copr-backend /bin/sh -c 'pgrep mockchain'`
	echo "pid2:"$PID2

        rlAssertEquals "Test that mockchain has been kept running" $PID1 $PID2
    ...
    rlPhaseStartCleanup
    	docker exec copr-backend /bin/sh -c 'rm -r /var/tmp/mockremote*'
    rlPhaseEnd
