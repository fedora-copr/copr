...
rlJournalStart
    rlPhaseStartTest RpmSigning
        ...
        tmpdir=`mktemp -d`
        cd $tmpdir
        dnf download --disablerepo=* --repofrompath=examplerepo,http://localhost:5002/results/@copr/copr-dev/fedora-23-x86_64 --enablerepo=examplerepo --refresh example # downloads rpm as well as srpm
        for rpmfile in *; do
