...
rlJournalStart
    ...
    rlPhaseStartTest
        ...
        rlRun "dnf -y copr enable $DNF_COPR_ID/${NAME_PREFIX}Createrepo"
        ...
        sleep 125
        rlRun "dnf -y copr enable $DNF_COPR_ID/${NAME_PREFIX}Createrepo fedora-rawhide-x86_64"
        rlRun "dnf --disablerepo='*' \
            --enablerepo='copr:${FRONTEND_PUBLIC_HOST}:$(repo_owner):${NAME_VAR}Createrepo' \
            list available 2>&1 | grep 'Failed to synchronize'" 1
    rlPhaseEnd
    ...
    rlPhaseStartCleanup
        ...
        rlRun "dnf -y copr remove $DNF_COPR_ID/${NAME_PREFIX}Createrepo"
    rlPhaseEnd
rlJournalPrintText
rlJournalEnd
