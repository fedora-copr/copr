...
@pytest.yield_fixture
def mc_check_output():
    with mock.patch("{}.check_output".format(MODULE_REF)) as handle:
        yield handle
...
class TestDistGitImporter(object):
    ...
    def test_before_git_import(self, mc_check_output):
        # dummy test, just for coverage
        self.dgi.before_git_import(self.task_1)
        assert mc_check_output.called

    def test_after_git_import(self, mc_check_output):
        # dummy test, just for coverage
